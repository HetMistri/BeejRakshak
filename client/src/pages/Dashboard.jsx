    ...
    const fetchForecast = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/forecast?${params}&appid=${API_KEY}&units=metric`,
        );
        if (res.ok) {
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pt-5 pb-1 flex items-center justify-between">
        <div>
          <h3 className="text-xl font-extrabold text-stone-800 tracking-tight">
            {monthName} {year}
          </h3>
...
function WeatherTab({ profile }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const aLat = cData.coord?.lat,
        aLon = cData.coord?.lon;
      if (aLat != null && aLon != null) {
        const aRes = await fetch(
          `https://api.openweathermap.org/data/2.5/air_pollution?lat=${aLat}&lon=${aLon}&appid=${API_KEY}`,
        );
      ...
      <div className="max-w-6xl animate-fade-in">
        <div className="rounded-2xl bg-amber-50 border border-amber-200 p-8 text-center">
          ...
          <p className="text-amber-600 text-sm mt-2 max-w-md mx-auto">
            The ML prediction server is not running. Start it with:
            <br />
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-stone-400">
                        {fmtKg(best.current_price)} × {recommendation.quantity}{" "}
                        kg
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-stone-400">
                        {best.distance_km.toFixed(0)} km × ₹5/km
                      </p>
                    </div>
                  </div>
...
function computeCropSuitability(crop, weather, season) {
  ...
  if (crop.season === season) {
    ...
    factors.push({
      name: "Season",
      value: cap(season),
      detail: `This is ${cap(crop.season)} crop`,
      good: true,
      pts: 30,
    });
  } else {
    ...
    const ci = order.indexOf(crop.season),
      si = order.indexOf(season);
    ...
    factors.push({
      name: "Season",
      value: cap(season),
      detail: `Best in ${cap(crop.season)} season`,
      good: false,
      pts,
    });
  }
  ...
  if (windSpeed < 15) {
    ...
    factors.push({
      name: "Wind",
      value: `${windSpeed.toFixed(0)} km/h`,
      detail: "High — risk for seedlings",
      good: false,
      pts: 0,
    });
  }
  ...
  const sunPts =
    crop.waterNeed === "High" ? (clouds > 30 ? 10 : 6) : clouds < 50 ? 10 : 5;
  score += sunPts;
  factors.push({
    name: "Sunlight",
    value: `${100 - clouds}% clear`,
    detail: clouds < 40 ? "Abundant" : "Moderate",
    good: sunPts >= 8,
    pts: sunPts,
  });
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mt-3">
            {farmerCropData.factors.map((f, fi) => (
              <div key={fi} className="text-center">
                <p
                  className={`text-xs font-bold ${f.good ? "text-emerald-600" : "text-amber-600"}`}
                >
                  {f.pts}pt
                </p>
        ...
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {topCrops.map((c, ci) => {
            ...
            return (
              <div
                key={ci}
                className={`rounded-xl border overflow-hidden hover-lift transition-all ${
                  isTop
      ...
      <div className="rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 text-xs text-stone-600">
          <div className="flex gap-2">
            ...
            <span>
              Temperature match — is current temp in the crop's optimal range?
            </span>
          </div>
          <div className="flex gap-2">
            <span className="text-emerald-600 font-extrabold shrink-0">
            ...
            <span>Humidity match — current humidity vs crop's ideal range</span>
          </div>
          <div className="flex gap-2">
            <span className="text-emerald-600 font-extrabold shrink-0">
              30pt
            ...
            <span>
              Season alignment — is this the right season for this crop?
            </span>
          </div>
          <div className="flex gap-2">
            <span className="text-emerald-600 font-extrabold shrink-0">
            ...
            <span>
              Wind conditions — calm winds better for sowing and spraying
            </span>
          </div>
          <div className="flex gap-2">
            <span className="text-emerald-600 font-extrabold shrink-0">
            ...
            <span>Sunlight — cloud cover vs crop's light requirements</span>
          </div>
        </div>
      </div>
    </div>
...
function AlertSystemTab({ profile }) {
  ...
  const RECENT_ALERTS = [
    ...
    },
    {
      ...
      text: `${crop} price at nearest mandi increased by ₹85/quintal.`,
      severity: "low",
    },
    {
      time: "3d ago",
